...
function mediaPolyfill () {
    if (false && isApiNotImplemented("saveImageToPhotosAlbum")) {
...
function uiPolyfill () {
    ...
    if (isApiNotImplemented("setTabBarItem")) {
        ...
        uni.setTabBarItem = function (options) {
            console.warn("api: uni.setTabBarItem 笔记设置 tabBar 某一项的内容 在当前平台不支持，执行失败")
			uni.showToast({
				title: "该功能在当前平台不支持35",
				icon:"none"
			})
    ...
    if (isApiNotImplemented("setTabBarStyle")) {
        ...
        uni.setTabBarStyle = function (options) {
            console.warn("api: uni.setTabBarStyle 笔记设置 tabBar 的整体样式 在当前平台不支持，回调成功")
			uni.showToast({
				title: "该功能在当前平台不支持36",
				icon:"none"
			})
    ...
    if (isApiNotImplemented("setBackgroundColor")) {
        ...
        uni.setBackgroundColor = function (options) {
            console.warn("api: uni.setBackgroundColor 笔记设置窗口的背景色 在当前平台不支持，执行失败")
			uni.showToast({
				title: "该功能在当前平台不支持43",
				icon:"none"
			})
    ...
    if (isApiNotImplemented("setBackgroundTextStyle")) {
        // 笔记设置下拉背景字体、loading 图的样式
        uni.setBackgroundTextStyle = function (options) {
            console.warn("api: uni.setBackgroundTextStyle 笔记设置下拉背景字体、loading 图的样式 在当前平台不支持，执行失败")
			uni.showToast({
				title: "该功能在当前平台不支持44",
				icon:"none"
			})
    ...
    if (isApiNotImplemented("loadFontFace")) {
        ...
        uni.loadFontFace = function (options) {
            console.warn("api: uni.loadFontFace 笔记加载网络字体 在当前平台不支持，执行失败")
			uni.showToast({
				title: "该功能在当前平台不支持47",
				icon:"none"
			})
