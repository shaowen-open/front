<template>
	<view class="u-album">
		<view class="u-album__row" ref="u-album__row" :style="{display: urls.length === 1 ? 'inline-block' : 'inline-flex', width: '100%'}"
			v-for="(arr, index) in showUrls" :forComputedUse="albumWidth" :key="index">
			<view class="u-album__row__wrapper" v-for="(item, index1) in arr" :key="index1"
				...
				<image :src="getSrc(item)" :mode="
                    ...
                    ]" :class="[{single: urls.length == 1}]"></image>
				<view v-if="
                        showMore &&
