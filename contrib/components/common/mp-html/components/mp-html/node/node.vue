<template>
  <view @tap="nodeTap" :id="attrs.id" :class="(opts[5] ? '_editable_node ' : '') + '_block _'+name+' '+attrs.class" :style="(ctrl.root?'border:1px dashed black;padding:5px;display:block;box-sizing: border-box;':'')+attrs.style">
    <block v-for="(n, i) in childs" v-bind:key="i">
      ...
      <text v-else-if="n.type==='text'&&ctrl['e'+i]===1" :data-i="i" style="min-width:710rpx;max-width:710rpx;display:block;box-sizing: border-box;" @tap.stop="editStart">{{n.text}}
        <text v-if="!n.text" style="min-width:710rpx;max-width:710rpx;box-sizing: border-box;">{{opts[6]||'请输入'}}</text>
      </text>
      <textarea v-else-if="n.type==='text'" style="min-width:710rpx;max-width:710rpx;box-sizing: border-box;color: #333333;" auto-height maxlength="-1" :focus="ctrl['e'+i]===3" :value="n.text" :data-i="i" @focus="editFocus" @input="editInput" @blur="editEnd" :show-confirm-bar="false"/>
...
myAudio,
  ...
  methods:{editStart (e) {
	...
	editFocus(e){
		...
		if( currentPage && e.detail.height){
		   currentPage.setTextareaBottom(e.detail.height);
		}
	},
	editInput (e) {
	  ...
	  if( currentPage){
	  	currentPage.setTextareaBottom(0);
	  }
    },insert (node) {
      setTimeout(() => {
    ...
    linkTap (e) {
      ...
      if (href) {
        ...
        } else if(href && href.indexOf('tel:') === 0){
			let phone = href.replace('tel:', '');
			console.log(phone);
			if(uni.wen.util.validatePhoneNumber(phone)){
				uni.wen.toUrl(22, phone, null);
			}else{
				uni.showToast({
					title: '手机号码不合法!',
					icon: 'none'
