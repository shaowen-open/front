<template>
  <view id="_root" :class="(selectable?'_select ':'') +'_root mphtml_scene_'+scene" :style="containerStyle" @tap="_containTap">
	<view class="post-flag-icon-wrap" v-if="scene != 3">
		<block>
			<i v-if="isAdminDelete" class="isAdminDelete-icon mini-icon mini-yisuoding"></i>
			<i v-else-if="isInReview == 0" class="isInReview-icon mini-icon mini-shenhezhong1"></i>
			<i v-else-if="isInReview == 2" class="reviewBack-icon mini-icon mini-beibohui"></i>
		</block>
		<i v-if="isSticky" class="isSticky-icon mini-icon mini-zhiding1"></i>
		<block>
			<i v-if="pay_type == 0" class="pay-coins-icon mini-icon mini-jinbi2"></i>
			<i v-else-if="pay_type == 1" class="pay-balance-icon mini-icon mini-xueyuan-fufeikecheng"></i>
			<i v-else-if="isSingle" class="single-icon mini-icon mini-tuwen1"></i>
		</block>
	</view>
...
import miniCustom from './mini-custom/index.js'
import editable from './editable/index.js'
const plugins=[audio,highlight,latex,imgCache,miniCustom,editable,]
// #ifdef APP-PLUS-NVUE
