	...
	<view  class="jsfun-record" @tap="showPicker">
	 ...
	 <view class="conbox record" :class="{'pickerShow':isShow}" >
		...
		<view class="record-box">
			...
			<canvas class="canvas" canvas-id="canvas" @touchstart="start" @longpress="record" @touchend="end" @touchmove.stop.prevent="miniMoveHandle">
	...
	export default {
		...
		mounted() {
			...
			query.exec(function(res){
				...
				//that.bili = res[0].width / 100;
				
				that.tempw = res[0].width;//使用canvas的宽度计算中心点的位置
				that.temph = res[0].height;
			})
			//根据中心图片的大小计算圆环的大小
			var query = uni.createSelectorQuery().in(that);
			query.select(".recording").boundingClientRect()
			query.exec(function(res){
				that.tempw1 = res[0].width;//使用点按图形的宽度计算圆环的宽高
			})
		...
		methods: {
			...
			okClick(){
				...
				this.$emit('okClick',this._voicePath, this.cacheRecordTime)
			...
			start() {
				...
				// #ifdef APP
				uni.wen.util.checkAppPermission('record', '[麦克风权限]获取您的录取声音等信息，用于发表语音笔记', () => {
					that._start()
				});
				// #endif
			...
			_start(){
				...
				this.cacheRecordTime = this.recordTime;
	...
	.jsfun-record {
       ...
       .mask {
        ...
        .record{		
			...
			.confirm{
				 @extend .btncom;
				background:url(@/pagesA/static/jc-record/confirm.png) no-repeat 100% 100%;
				background-size: 100% 100%;
			}
